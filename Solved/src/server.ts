...
async function viewEmployees() {
  const { rows } = await pool.query(
    `SELECT employees.id, employees.first_name, employees.last_name, role.title, departments.name AS department, role.salary, CONCAT(managers.first_name, ' ', managers.last_name) AS manager
     FROM employees
...
async function addRole() {
  const { rows: department } = await pool.query(
